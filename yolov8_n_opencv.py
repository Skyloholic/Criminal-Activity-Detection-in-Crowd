# import random

# import cv2
# import numpy as np
# from ultralytics import YOLO

# # opening the file in read mode
# my_file = open("utils/coco.txt", "r")
# # reading the file
# data = my_file.read()
# # replacing end splitting the text | when newline ('\n') is seen.
# class_list = data.split("\n")
# my_file.close()

# # print(class_list)

# # Generate random colors for class list
# detection_colors = []
# for i in range(len(class_list)):
#     r = random.randint(0, 255)
#     g = random.randint(0, 255)
#     b = random.randint(0, 255)
#     detection_colors.append((b, g, r))

# # load a pretrained YOLOv8n model
# model = YOLO("weights/yolov8n.pt", "v8")

# # Vals to resize video frames | small frame optimise the run
# frame_wid = 640
# frame_hyt = 480

# # cap = cv2.VideoCapture(1)
# cap = cv2.VideoCapture("inference/videos/afriq1.MP4")

# if not cap.isOpened():
#     print("Cannot open camera")
#     exit()

# while True:
#     # Capture frame-by-frame
#     ret, frame = cap.read()
#     # if frame is read correctly ret is True

#     if not ret:
#         print("Can't receive frame (stream end?). Exiting ...")
#         break

#     #  resize the frame | small frame optimise the run
#     # frame = cv2.resize(frame, (frame_wid, frame_hyt))

#     # Predict on image
#     detect_params = model.predict(source=[frame], conf=0.45, save=False)

#     # Convert tensor array to numpy
#     DP = detect_params[0].numpy()
#     print(DP)

#     if len(DP) != 0:
#         for i in range(len(detect_params[0])):
#             print(i)

#             boxes = detect_params[0].boxes
#             box = boxes[i]  # returns one box
#             clsID = box.cls.numpy()[0]
#             conf = box.conf.numpy()[0]
#             bb = box.xyxy.numpy()[0]

#             cv2.rectangle(
#                 frame,
#                 (int(bb[0]), int(bb[1])),
#                 (int(bb[2]), int(bb[3])),
#                 detection_colors[int(clsID)],
#                 3,
#             )

#             # Display class name and confidence
#             font = cv2.FONT_HERSHEY_COMPLEX
#             cv2.putText(
#                 frame,
#                 class_list[int(clsID)] + " " + str(round(conf, 3)) + "%",
#                 (int(bb[0]), int(bb[1]) - 10),
#                 font,
#                 1,
#                 (255, 255, 255),
#                 2,
#             )

#     # Display the resulting frame
#     cv2.imshow("ObjectDetection", frame)

#     # Terminate run when "Q" pressed
#     if cv2.waitKey(1) == ord("q"):
#         break

# # When everything done, release the capture
# cap.release()
# cv2.destroyAllWindows()
import random
import cv2
import numpy as np
from ultralytics import YOLO

# Open the COCO classes file
my_file = open("utils/coco.txt", "r")
# Read and split the text by newline
class_list = my_file.read().split("\n")
my_file.close()

# Generate random colors for class list
detection_colors = []
for i in range(len(class_list)):
    r = random.randint(0, 255)
    g = random.randint(0, 255)
    b = random.randint(0, 255)
    detection_colors.append((b, g, r))

# Load a pretrained YOLOv8n model
model = YOLO("weights/yolov8n.pt", "v8")

# Set frame dimensions for resizing
frame_wid = 640
frame_hyt = 480

# Initialize video capture
cap = cv2.VideoCapture("inference/videos/afriq0.MP4")

if not cap.isOpened():
    print("Cannot open camera")
    exit()

while True:
    # Capture frame-by-frame
    ret, frame = cap.read()
    if not ret:
        print("Can't receive frame (stream end?). Exiting ...")
        break

    # Resize the frame for optimization
    frame = cv2.resize(frame, (frame_wid, frame_hyt))

    # Predict on the frame
    detect_params = model.predict(source=[frame], conf=0.45, save=False)

    # Convert tensor array to numpy
    DP = detect_params[0].numpy()

    person_count = 0  # Initialize person counter

    if len(DP) != 0:
        for i in range(len(detect_params[0])):
            boxes = detect_params[0].boxes
            box = boxes[i]
            clsID = int(box.cls.numpy()[0])  # Class ID of the detected object
            conf = box.conf.numpy()[0]
            bb = box.xyxy.numpy()[0]

            # Check if the detected object is a person (usually class ID 0 in COCO dataset)
            if clsID == 0:  # Adjust this if "person" class ID differs
                person_count += 1  # Increment person count

            # Draw bounding box and display class name and confidence
            cv2.rectangle(
                frame,
                (int(bb[0]), int(bb[1])),
                (int(bb[2]), int(bb[3])),
                detection_colors[clsID],
                3,
            )

            font = cv2.FONT_HERSHEY_COMPLEX
            cv2.putText(
                frame,
                f"{class_list[clsID]}",
                (int(bb[0]), int(bb[1]) - 10),
                font,
                1,
                (255, 255, 255),
                2,
            )

    # Display the person count on the frame
    cv2.putText(
        frame,
        f"Person count: {person_count}",
        (10, 30),
        cv2.FONT_HERSHEY_SIMPLEX,
        1,
        (0, 255, 0),
        2,
    )

    # Display the resulting frame
    cv2.imshow("Object Detection", frame)

    # Terminate run when "Q" is pressed
    if cv2.waitKey(1) == ord("q"):
        break

# Release the capture and close windows
cap.release()
cv2.destroyAllWindows()
# {round(conf, 3)}%